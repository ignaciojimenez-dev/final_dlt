...
def load_config(path: str):
    ...
    try:
        ...
        log.error(f"❌ Error fatal leyendo el JSON en {path}: {e}")
        raise e
    ...
    for flow in dataflows:
        ...
        for sink in sinks:
            try:
                ...
                if not output_path:
                    log.warning(f"⚠️ Sink para '{input_table_name}' sin path. Omitiendo.")
                    fail_count += 1
                    continue
                ...
                log.error(f"   ❌ Tabla no encontrada: {full_source_table}")
                fail_count += 1
            except Exception as e:
                log.error(f"   ❌ Error en sink '{sink.get('input')}': {e}")
                fail_count += 1
...
if __name__ == "__main__":
    # ==============================================================================
    # LECTURA DE PARÁMETROS (NATIVO DATABRICKS)
    # ==============================================================================
    path_arg = None
    schema_arg = None
    ...
    if len(sys.argv) > 2:
        ...
        log.info("✅ Parámetros detectados vía sys.argv (Modo Job)")

    # 2. MODO INTERACTIVO (dbutils widgets)
    # Solo si no se encontraron en sys.argv y dbutils está disponible
    if (not path_arg or not schema_arg) and dbutils:
        try:
            log.info("⚠️ No hay argumentos de Job. Intentando leer Widgets...")
            ...
            # Solo usamos los widgets si tienen contenido (no strings vacíos)
    ...
    if not path_arg or not schema_arg:
        log.error("❌ ERROR FATAL: Faltan parámetros.")
